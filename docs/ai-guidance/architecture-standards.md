...
| Idle animation | `utils/animationPresets.ts` + raw `gsap`                                           | raw `gsap`                                                           | **in force** — `IdleAnimator` (§5); the last un-killed `repeat: -1` was closed in `c8cacc5`; `animationPresets.ts` reached 0 importers against `animationHelpers.ts`'s 11 and was deleted (§10)                                                                                                                |
...
| 2     | ParticleEngine                       | **in force** — `utils/particles/{engine,presets,texture,registry}.ts`; one clock-driven, scope-disposed engine per scene reached via `getParticleEngine(scene)`; all 3 legacy modules deleted. **66 `emit`/`stream` call sites across 40 files** (`grep -rn "\.emit(PARTICLES\|\.stream(PARTICLES" src/`) — the "~35" in the original plan predated the owl and celebration migrations, and the "54 across 33" of the previous revision predated Round 6. Contract test in `tests/particles/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 3     | IdleAnimator                         | **in force.** `utils/idle/{idleAnimator,registry}.ts` is reached via `getIdleAnimator(scene)`, wired into both shells, contract-tested in `tests/framework/`. Decor idles use presets (spinningTop, hangingMobile, catPlush, rubberDuck, toyBall, toyTrain's orbit spin) and `register()` covers the keyframe loops (hoppingChick, windUpMouse, toy cars ×2, musicPlayer, animalVisitors). The last un-killed leak — `playroom/floorToys/toyTrain.ts`'s puff tween and self-rescheduling horn — was closed in `c8cacc5` and is pinned by `tests/room/playroom-timer-ownership.test.mjs`; the rule it produced is in §5. The other raw `repeat: -1` sites (owl `entities/owl/idle.ts`, raccoon, snail, skyBackdrop, gamePortal, `living-room/decor/fireplace.ts:137`/`:162`, `animalVisitors.ts:622`, `utils/animationHelpers.ts:189`) each self-manage via a returned cleanup, a `cleanups` array, or a tracked handle — verified individually, not assumed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `createDisposeCollector` | **adapter** — `sceneHelpers.ts:405` builds a `DisposalScope` internally, so its 5 callers get LIFO order, idempotence and exception isolation without knowing it. Safe to leave |
| `disposeSceneResources`  | **independent** — still called by `worldSceneFactory.ts:135` and `roomSceneFactory.ts:162`                                                                                      |
...
importers, sitting one directory away from `animationHelpers.ts`'s eleven. A bet
...
export interface LightingDescriptor {
  ...
  /**
   * Optional up-facing bounce. Modelled on light returning off a real floor —
   * the Kitchen and Living Room both set it, and `sceneHelpers` maps a
   * `LightingConfig`'s `bounceIntensity` / `bounceColor` onto it.
   */
...
export interface LightingRig {
  ...
  /** Null when the descriptor supplied no `bounce` block. */
...
**Known limitation — shadows are not optional.** `lightingRig.ts:100` sets
...
room viewport 0.5%, Nature 0.3%, core visibility ≥ 0.866 in every room (same probe run as §Status; the two cells gave 0.866 and 0.868 for one measurement until 2026-08-01).
...
dispatcher) **and the visible half too**: `roomSceneFactory.ts:152` installs the
same `createMissAcknowledgement(scene)`. The fixed-depth sparkle that could land
inside a wall is why the acknowledgement anchors on the raycast hit point and
stands off 0.45 units along the surface normal rather than choosing a depth.
...
- The contract test **does not execute the builder**.
  `tests/framework/sceneDescriptor.test.mjs:53` reads `buildScene.ts` with
...
the other seventeen reach the animator through `utils/idle/registry.ts`, which is
...
`utils/sceneHelpers.ts:405`; `createWorldTapDispatcher` is a thin adapter that
constructs `createInteractionController` at `utils/worldTapDispatcher.ts:45`,
and `createTapInteraction`'s 17 prop-file callers flow through it. Three names
...
delegation edges themselves. Four states are in use: `inverted` (adoption total,
...
| `tests/minigames/`         | 9     | per-game logic — eight of the nine are Little Shark (regions, aim, dodge, frenzy, agency, HUD, celebration, rig); the ninth is `balanceCurvesWired`                                                                                                                                                                                                                                                                            |
...
`package.json`'s `test` script is a single **recursive glob** —
`node --test "tests/**/*.test.mjs"` — so a new suite is picked up automatically
wherever it lands, including in a brand-new directory. Do not replace it with a
directory list: `scripts/precommit-check.cjs` explains at length why an
enumeration is an exclusion criterion doing unchecked work, and
`tests/framework/ciWorkflow.test.mjs` now fails the build if either the script
or the CI workflow starts naming directories. (Until 2026-08-01 `ci.yml` did
exactly that, and ran 145 of 429 tests.)
...
1. **§7 — the camera split, the last unification that did not invert.** The 3
   ...
   matters. Write the test first. Tracked as `camera` in
