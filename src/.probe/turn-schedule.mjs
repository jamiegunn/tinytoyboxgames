...
const M = await bundleEntry('turn-schedule', `
...
const POSE = {
  playroom: { polar: 0.90, d: 5.0, ty: 3.75, tz: -8.0 },
...
const ASPECTS = [];
for (let a = 0.40; a <= 2.601; a += 0.02) ASPECTS.push(+a.toFixed(2));
...
for (const [id, fn, L] of [['playroom', M.buildPlayroomContents, M.PLAYROOM], ['kitchen', M.buildKitchenContents, M.KITCHEN], ['living-room', M.buildLivingRoomContents, M.LIVING]]) {
  ...
  const ground = contents.floorTargets?.[0]; const props = []; const sn = new Set();
  for (const t of tap) { if (t === ground || sn.has(t)) continue; sn.add(t); const b = new Box3().setFromObject(t); if (b.isEmpty()) continue;
  ...
  const c = POSE[id]; const pivot = new Vector3(0, c.ty, c.tz);
  ...
  for (const aspect of ASPECTS) {
    let lo = 0, hi = 1.4;
    for (let i = 0; i < 14; i++) { const mid = (lo + hi) / 2; if (dirtyAt(mid, aspect)) hi = mid; else lo = mid; }
    const safe = lo;
    let need = 0, miss = false;
    for (const ex of exits) {
      ...
      if (!Number.isFinite(b)) { miss = true; break; }
...
for (const aspect of ASPECTS) {
  ...
  if (cells.some((c) => c.miss)) { console.log(`  ${aspect.toFixed(2)}   UNREACHABLE in ${Object.keys(TABLE).filter((id) => TABLE[id][aspect].miss).join(',')}`); continue; }
  const need = Math.max(...cells.map((c) => c.need));
  const safe = Math.min(...cells.map((c) => c.safe));
  rows.push({ aspect, need, safe });
  console.log(`  ${aspect.toFixed(2)}   ` + cells.map((c) => `${((c.need * 180) / Math.PI).toFixed(1)}`).join(' / ').padEnd(24)
    + `±${((need * 180) / Math.PI).toFixed(1)}°`.padStart(9) + `±${((safe * 180) / Math.PI).toFixed(1)}°`.padStart(12) + `   ${safe > need ? (safe / need).toFixed(2) + 'x' : 'NONE'}`);
}
