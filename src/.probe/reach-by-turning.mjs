...
const M = await bundleEntry('reach-by-turning', `
...
const ASPECTS = [0.40, 0.46, 0.56, 0.70, 0.85, 1.0, 1.2, 1.4, 1.78, 2.37];
const CLAMP = 6.0;
const noop = () => {};
...
for (const [id, fn, L] of [['playroom', M.buildPlayroomContents, M.PLAYROOM], ['kitchen', M.buildKitchenContents, M.KITCHEN], ['living-room', M.buildLivingRoomContents, M.LIVING]]) {
  ...
  for (const aspect of ASPECTS) {
    let best = null;
    for (let d = 4; d <= 20.1; d += 0.5) for (let polar = 0.80; polar <= 1.501; polar += 0.02) for (let ty = 0; ty <= 3.51; ty += 0.25) for (let tz = shell.frontZ + 1; tz <= shell.backZ - 1.01; tz += 1) {
      const pivot = new Vector3(0, ty, tz);
      const orbit = { azimuth: preset.azimuth, pivot, radii: [d], polars: [Math.max(0.75, polar - 0.1), polar, Math.min(1.5, polar + 0.1)], ceilingClamp: CLAMP };
      const dirty = (r) => { for (const pos of M.orbitPositionsAt(r, orbit)) { aim(pos, pivot, aspect); if (M.frameSeesPastWalls(pos, corners(), shell)) return true; } return false; };
      if (dirty(0)) continue;
      ...
      let lo = 0, hi = 0.9;
      for (let i = 0; i < 12; i++) { const mid = (lo + hi) / 2; if (dirty(mid)) hi = mid; else lo = mid; }
      const safe = lo;
      // Smallest turn that brings each exit fully inside the frame.
      let worst = 0, unreachable = false;
      for (const ex of exitProps) {
        ...
        for (let r = -safe; r <= safe + 1e-9; r += Math.max(safe / 24, 1e-3)) {
          ...
          if (ndcOf(ex.pts) <= 0.97) { needed = Math.min(needed, Math.abs(r)); }
        }
        if (!Number.isFinite(needed)) { unreachable = true; break; }
    ...
    if (!best) { console.log(`  ${aspect.toFixed(2)}    none — no pose reaches every exit within its own safe turn`); continue; }
    console.log(`  ${aspect.toFixed(2)}    ±${((best.safe * 180) / Math.PI).toFixed(1)}°`.padEnd(22) + `±${((best.worst * 180) / Math.PI).toFixed(1)}°`.padStart(8) + `   ${best.worst <= best.safe ? 'REACHABLE' : 'no'}   d ${best.d.toFixed(1)} polar ${best.polar.toFixed(2)} target [0, ${best.ty.toFixed(2)}, ${best.tz.toFixed(1)}]`);
  }
  contents?.cleanup?.();
}
process.exit(0);
