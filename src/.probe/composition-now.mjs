...
for (const [id, fn] of [
  ...
  for (const aspect of ASPECTS) {
    ...
    const t = { props: 0, soft: 0, wall: 0, floor: 0, ceiling: 0, nothing: 0 };
    const n = 32;
    ...
    const tot = n * n;
    const c = Object.fromEntries(Object.entries(t).map(([a, b]) => [a, b / tot]));
    minProps = Math.min(minProps, c.props);
    maxCeil = Math.max(maxCeil, c.ceiling);
    maxVoid = Math.max(maxVoid, c.nothing);
    maxBare = Math.max(maxBare, c.floor);
    if (aspect <= 1.0) {
      minPropsP = Math.min(minPropsP, c.props);
      maxBareP = Math.max(maxBareP, c.floor);
    }
    console.log(
      `  ${aspect.toFixed(2)}  objects ${(c.props * 100).toFixed(1)}%  rug ${(c.soft * 100).toFixed(1)}%  bare floor ${(c.floor * 100).toFixed(1)}%  wall ${(c.wall * 100).toFixed(1)}%  ceiling ${(c.ceiling * 100).toFixed(1)}%  void ${(c.nothing * 100).toFixed(1)}%`,
    );
  }
  console.log(
    `  --> ALL aspects: objects >= ${(minProps * 100).toFixed(1)}%  bare floor <= ${(maxBare * 100).toFixed(1)}%  ceiling <= ${(maxCeil * 100).toFixed(1)}%  void <= ${(maxVoid * 100).toFixed(1)}%`,
  );
