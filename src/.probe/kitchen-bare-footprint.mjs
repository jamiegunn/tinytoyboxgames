...
for (const [label, aspect] of [
  ...
  for (let iy = 0; iy < n; iy++) {
    for (let ix = 0; ix < n; ix++) {
      ...
      if (!isFloor) {
        if (!name.includes('wall') && !name.includes('ceiling') && !name.includes('wainscot')) props++;
        continue;
      }
  ...
  const xs = bare.map((b) => b.x),
    zs = bare.map((b) => b.z);
  console.log(
    `  it lands on x [${Math.min(...xs).toFixed(1)}, ${Math.max(...xs).toFixed(1)}]  z [${Math.min(...zs).toFixed(1)}, ${Math.max(...zs).toFixed(1)}]`,
  );
  // The bottom third of the frame is what reads as "empty foreground".
  const low = bare.filter((b) => b.frameY < 0.34);
  if (low.length) {
    const lz = low.map((b) => b.z),
      lx = low.map((b) => b.x);
    console.log(
      `  bottom third of frame: ${low.length} rays, landing on x [${Math.min(...lx).toFixed(1)}, ${Math.max(...lx).toFixed(1)}]  z [${Math.min(...lz).toFixed(1)}, ${Math.max(...lz).toFixed(1)}]`,
    );
  }
