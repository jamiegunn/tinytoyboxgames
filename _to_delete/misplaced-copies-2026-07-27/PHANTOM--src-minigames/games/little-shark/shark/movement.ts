...
/**
 * Upper bound on how fast the idle figure-eight can carry the shark.
 *
 * The drift target is (cx + 1.5 sin p, cz + 1.5 sin 2p) with p advancing at
 * 0.4 rad/s, so the target's speed is 1.5 * 0.4 * sqrt(cos^2 p + 4 cos^2 2p),
 * which peaks at 1.5 * 0.4 * sqrt(5) = 1.342 units/s. The shark chases that
 * target with a critically damped spring, and a critically damped spring has
 * no resonant gain, so it cannot outrun what it is chasing.
 *
 * 2.0 therefore sits above every speed idle drift can produce and below the
 * release glide, which starts at up to DRAG_RELEASE_MAX_SPEED = 9.
 */
const IDLE_DRIFT_MAX_SPEED = 2.0;

/**
 * Whether the shark is moving because the child moved it.
 *
 * The collision path eats any fish that comes within FISH_HIT_RADIUS, and that
 * was never gated on input, so the shark harvested the reef on its own. It is
 * not a small effect: a probe that loaded the game and then touched nothing at
 * all scored 24 out of 24 slots, and a 60-second idle recording gained 72
 * points while scoring in 49% of all one-second intervals. The reef was not
 * merely playable without the child, it was faster without the child.
 *
 * The mechanism is that the two halves of the design point at each other. Idle
 * drift orbits the shark inside a 3x3 figure-eight, and `spawnNearShark` aims
 * every replacement fish at a point 2-8 units from the shark — which, because
 * the camera looks at the shark, is the same instruction as "aim at the middle
 * of the screen", i.e. at the exact spot a child pokes. So the game delivers
 * fish to the shark's mouth and the mouth is always open.
 *
 * Dose-response confirms it is a flow process linear in density: at the old
 * MIN/MAX_FISH_COUNT of 5/8 the idle rate was 30 points/minute, and at 14/18 it
 * was 72 — so raising the density in round 1 amplified a defect that was
 * already shipping rather than introducing one.
 *
 * The fix is not to aim the fish elsewhere, because the place they are aimed is
 * the place the child taps and that part is correct. It is to stop the shark
 * scoring when nobody asked it to. Contact still eats when the child is
 * dragging the shark, when the shark is gliding out of a release, and when it
 * is lunging at a fish the child tapped — all three are the child's doing.
 * Idle drift keeps swimming and keeps looking alive; it just stops collecting.
 *
 * @param state - Shark movement state.
 * @returns True when contact should be allowed to count as a catch.
 */
export function isPlayerDriven(state: SharkMoveState): boolean {
  return state.isBeingDragged || state.swimPhase !== 'idle' || getSpeed(state) > IDLE_DRIFT_MAX_SPEED;
}
