...
/**
 * A follow target for a particle emitter: either an Object3D whose world
 * position is tracked, or a getter returning the current emitter position.
 */
export type EmitterFollowTarget = Object3D | (() => Vector3);
...
export class SimpleParticleSystem {
  ...
  private follow: EmitterFollowTarget | null = null;
  ...
  configure(opts: {
    ...
    follow?: EmitterFollowTarget | null;
  }): void {
    ...
    if (opts.follow !== undefined) {
      ...
      this.updateEmitterFromFollow();
    }
  }

  /** Syncs the emitter position from the follow target, if one is set. */
  private updateEmitterFromFollow(): void {
    if (!this.follow) return;
    if (typeof this.follow === 'function') {
      this.emitterPosition.copy(this.follow());
    } else {
      this.follow.getWorldPosition(this.emitterPosition);
    }
  }
  ...
  private tick = (): void => {
    ...
    // Track the follow target so newly emitted particles spawn at the
    // emitter's current position rather than its configure-time position.
    this.updateEmitterFromFollow();
